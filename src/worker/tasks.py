...
async def summarize_text_with_pybreaker(content: str) -> str:
    ...
    try:
        ...
        try:
            from src.worker.openrouter_state import OpenRouterStateManager
        except ImportError:
        ...
        if "status_code=" in msg:
            try:
                ...
                pass
        elif "OpenRouter API error:" in msg:
            try:
                # Extract code from "OpenRouter API error: 400"
                code_str = msg.split("OpenRouter API error:")[1].strip()
                code = int(code_str)
            except (IndexError, ValueError):
                pass
    ...
    try:
        ...
        if "status_code=" in msg:
            try:
                ...
                pass
        elif "PDF extraction API error:" in msg or "OpenRouter API error:" in msg:
            try:
                parts = msg.split(":")
...
def process_task(self: Task, task_id: str) -> str:
    ...
    async def _run_task():
        ...
        # We cannot use the global worker pool because it might be bound to a different/closed loop
    ...
    async def _handle_error(exc, error_type="TransientError"):
        ...
        # Create a fresh Redis connection for error handling
        redis_conn = aioredis.from_url(settings.redis_url, decode_responses=True)
