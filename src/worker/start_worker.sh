...
check_redis() {
    ...
    if python3 -c "import redis; r=redis.from_url('${REDIS_URL:-redis://localhost:6379/0}'); r.ping(); print('Redis OK')" 2>/dev/null; then
...
start_consumer() {
    ...
    python3 src/worker/consumer.py &
    CONSUMER_PID=$!
...
start_worker_with_retry() {
    ...
    for attempt in $(seq 1 $max_retries); do
        ...
        log "Starting worker with command: ${CELERY_ARGS[*]}"
...
cleanup() {
    ...
    pkill -f "src/worker/consumer.py" 2>/dev/null || true
