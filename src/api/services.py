...
class TaskService:
    ...
    async def get_task_metrics(self, minutes: int = 60) -> TaskMetricsResponse:
        """
        Get task metrics (throughput) for the last N minutes.
        Buckets data by second for high-resolution charting.
        """
        now = datetime.utcnow()
        start_time = now.timestamp() - (minutes * 60)
        
        # Initialize buckets
        # Map timestamp (epoch seconds) -> {created: 0, completed: 0, failed: 0}
        buckets: Dict[int, Dict[str, int]] = {}
        
        # Helper to get bucket (bucket by minute)
        def get_bucket(ts_val: float):
            # Round down to nearest minute to reduce data points for longer durations
            ts_int = int(ts_val // 60) * 60
            if ts_int < start_time:
                return None
            if ts_int not in buckets:
                buckets[ts_int] = {"created": 0, "completed": 0, "failed": 0}
            return buckets[ts_int]

        # Scan all tasks (Note: In production with millions of tasks, this needs 
        # a dedicated time-series DB or Redis TimeSeries module. For this project,
        # scanning is acceptable as per requirements).
        async for key in self.redis.scan_iter("task:*"):
            task_data = await self.redis.hgetall(key)
            if not task_data:
                continue
            ...
            if task_data.get("created_at"):
                try:
                    created_at = datetime.fromisoformat(task_data["created_at"])
                    bucket = get_bucket(created_at.timestamp())
                    if bucket:
                        bucket["created"] += 1
                except (ValueError, TypeError):
                    pass
            ...
            if task_data.get("completed_at"):
                try:
                    ...
                    bucket = get_bucket(completed_at.timestamp())
                    if bucket:
                        bucket["completed"] += 1
                except (ValueError, TypeError):
                    pass
                    
            # Count failed (using updated_at for failed tasks as proxy for failure time)
            if task_data.get("state") in [TaskState.FAILED.value, TaskState.DLQ.value]:
                if task_data.get("updated_at"):
                     try:
                        updated_at = datetime.fromisoformat(task_data["updated_at"])
                        bucket = get_bucket(updated_at.timestamp())
                        if bucket:
                            bucket["failed"] += 1
                     except (ValueError, TypeError):
                        pass

        # Convert buckets to list, sorted by timestamp
        sorted_timestamps = sorted(buckets.keys())
        metrics_list = []
        
        # Fill in gaps with zeros if needed, or just return sparse data
        # For a smooth chart, we usually want at least the last few minutes contiguous
        # But for "realness", let's just return what we have plus the current second
        
        # Ensure current minute exists to show "live" activity (even if 0)
        now_ts = int(now.timestamp() // 60) * 60
        if now_ts not in buckets:
            buckets[now_ts] = {"created": 0, "completed": 0, "failed": 0}
            sorted_timestamps.append(now_ts)
            sorted_timestamps.sort()

        for ts in sorted_timestamps:
            # Use UTC timezone to ensure frontend parses it correctly as UTC
            from datetime import timezone
            dt_str = datetime.fromtimestamp(ts, tz=timezone.utc).isoformat()
            metrics_list.append(
                TaskMetrics(
                    timestamp=dt_str,
                    created_count=buckets[ts]["created"],
                    completed_count=buckets[ts]["completed"],
                    failed_count=buckets[ts]["failed"]
                )
            )
            
        return TaskMetricsResponse(metrics=metrics_list, time_window=f"{minutes}m")

    async def get_task_latency_metrics(self, minutes: int = 60) -> TaskLatencyResponse:
        """
        Get task latency metrics (P50, P95, P99) for the last N minutes.
        Buckets data by minute.
        """
        now = datetime.utcnow()
        start_time = now.timestamp() - (minutes * 60)
        
        # Initialize buckets
        # Map timestamp (epoch minutes) -> list of durations
        buckets: Dict[int, List[float]] = {}
        
        # Helper to get bucket (floor to minute)
        def get_bucket_ts(ts_val: float) -> int:
            return int(ts_val // 60) * 60
        ...
        async for key in self.redis.scan_iter("task:*"):
            ...
            # Only care about completed tasks for latency
            if task_data.get("state") != TaskState.COMPLETED.value:
                continue

            try:
                if task_data.get("created_at") and task_data.get("completed_at"):
                    created_at = datetime.fromisoformat(task_data["created_at"])
                    completed_at = datetime.fromisoformat(task_data["completed_at"])
                    ...
                    bucket_ts = get_bucket_ts(ts)
                    if bucket_ts not in buckets:
                        buckets[bucket_ts] = []
                    buckets[bucket_ts].append(duration)
            except (ValueError, TypeError):
                continue
        ...
        # Ensure we have at least one data point or the current minute
        current_minute = get_bucket_ts(now.timestamp())
        if current_minute not in buckets:
            buckets[current_minute] = []
            sorted_timestamps.append(current_minute)
            sorted_timestamps.sort()
            
        for ts in sorted_timestamps:
            durations = buckets[ts]
            count = len(durations)
            ...
            if count > 0:
                durations.sort()
                import numpy as np # Might be overkill, let's do simple math or ensure numpy is available. 
                # Actually, avoid numpy dependency if not already there. Use simple percentile logic.
        ...
        return TaskLatencyResponse(metrics=metrics_list, time_window=f"{minutes}m")
