...
async def list_tasks(
    status: Optional[TaskState] = Query(
...
@router.get("/metrics/throughput", response_model=TaskMetricsResponse)
async def get_task_metrics(
    ...
    Returns task creation, completion, and failure counts bucketed by second
    for the specified time window (default 60 minutes).
    Useful for visualizing real-time load and processing performance.
    """
    try:
        return await task_svc.get_task_metrics(minutes=minutes)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.get("/metrics/latency", response_model=TaskLatencyResponse)
async def get_task_latency_metrics(
    minutes: int = Query(60, description="Time window in minutes"),
    task_svc: TaskService = Depends(get_task_service),
) -> TaskLatencyResponse:
    """
    Get task latency metrics (P50, P95, P99).

    Returns task duration percentiles bucketed by minute
    for the specified time window (default 60 minutes).
    Useful for visualizing processing performance variability.
    """
    try:
        return await task_svc.get_task_latency_metrics(minutes=minutes)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get task latency metrics: {str(e)}",
        )
